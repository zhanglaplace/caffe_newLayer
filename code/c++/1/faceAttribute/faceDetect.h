...
public:
	FaceDetect();
	vector<FaceInfos> Detect(const cv::Mat& img, const int min_size, const float* threshold, const float factor, const int stage,bool landmark = false);
protected:
	vector<FaceInfos> ProposalNet(const cv::Mat& img, int min_size, float threshold, float factor);
	vector<FaceInfos> NextStage(const cv::Mat& image, vector<FaceInfos> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold,bool landmark);
