...
// 备注:
/*******************************************************/
float FaceDetect::IoU(float xmin, float ymin, float xmax, float ymax,
...
// 备注:
/*******************************************************/
std::vector<FaceInfos> FaceDetect::NMS(std::vector<FaceInfos>& bboxes,
	float thresh, char methodType) {
	std::vector<FaceInfos> bboxes_nms;
	if (bboxes.size() == 0) {
		return bboxes_nms;
	}
	std::sort(bboxes.begin(), bboxes.end(), CompareBBox); // 所有人脸按照scores得分进行排序
...
#pragma omp parallel for num_threads(threads_num)
		for (int32_t i = select_idx; i < num_bbox; i++) { // 遍历其他的目标
			if (mask_merged[i] == 1)
				continue;
			...
			// 求解IOU A∩B = area_intersect, A = area1 ,B = area2
			switch (methodType) {
			case 'u': // 
...
// 备注:
/*******************************************************/
void FaceDetect::BBoxRegression(vector<FaceInfos>& bboxes) {
#pragma omp parallel for num_threads(threads_num)
...
// 备注:防止图片越界
/*******************************************************/
void FaceDetect::BBoxPad(vector<FaceInfos>& bboxes, int width, int height) {
#pragma omp parallel for num_threads(threads_num)
	for (int i = 0; i < bboxes.size(); ++i) {
		FaceBox &bbox = bboxes[i].bbox;
...
// 备注:
/*******************************************************/
void FaceDetect::BBoxPadSquare(vector<FaceInfos>& bboxes, int width, int height) {
#pragma omp parallel for num_threads(threads_num)
	for (int i = 0; i < bboxes.size(); ++i) {
		FaceBox &bbox = bboxes[i].bbox;
		float w = bbox.xmax - bbox.xmin + 1;
...
// 备注:
/*******************************************************/
void FaceDetect::GenerateBBox(Blob<float>* confidence, Blob<float>* reg_box,
	...
	const float* confidence_data = confidence->cpu_data() + spatical_size; // 输出的特征向量
...
// 备注:Pnet过程，生成初步候选
/*******************************************************/
vector<FaceInfos> FaceDetect::ProposalNet(const cv::Mat& img, int minSize, float threshold, float factor) {
	cv::Mat  resized;
	int width = img.cols;
	int height = img.rows;
	...
	while (minWH >= 12) { // 尺度金字塔
		scales.push_back(scale);
		minWH *= factor;
		scale *= factor;
	}
	Blob<float>* input_layer = PNet_->input_blobs()[0];
	total_boxes_.clear(); //总face数目
	// 图像尺度金字塔，完成resize后forward
	for (int i = 0; i < scales.size(); i++) { 
		...
		GenerateBBox(confidence, reg, scales[i], threshold); // 得到候选区域
		std::vector<FaceInfos> bboxes_nms = NMS(candidate_boxes_, 0.5, 'u'); //非极大值抑制
...
/*******************************************************/
vector<FaceInfos> FaceDetect::NextStage(const cv::Mat& image, vector<FaceInfos> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold,bool landmark) {
...
// 备注:
/*******************************************************/
vector<FaceInfos> FaceDetect::Detect(const cv::Mat& image, const int minSize, const float* threshold, const float factor, const int stage, bool landmark) {
	...
	if (stage >= 2 && pnet_res.size()>0){
		...
		for (int iter = 0; iter < size; ++iter){
			...
			// 每次处理第一步后的mini_batch的rect
			vector<FaceInfos> input(pnet_res.begin() + start, pnet_res.begin() + end); 
			vector<FaceInfos> res = NextStage(image, input, 24, 24, 2, threshold[1],landmark);//Rnet
	...
	if (stage >= 3 && rnet_res.size()>0){
		...
		for (int iter = 0; iter < size; ++iter){
			...
			vector<FaceInfos> res = NextStage(image, input, 48, 48, 3, threshold[2],landmark);
